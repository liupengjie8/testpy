...
def save_rank_list(rank_list, year):
    ...
    for i, hospital in enumerate(rank_list):
        if i >= 2:
            ...
            scientific_value = col_list[3].getText().strip()
            rate = ''
            if col_list[4].findAll('span'):
                ...
                if col_list[4].findAll('span')[1].get('class') == 'arrow-ranking arrow-top col-md-6 col-sm-6 col-xs-6 ' \
                                                                  'text-left':
                    rate = '+' + str(rate)
            sql = 'insert into profession_scientific_best(rank, province, city, scientific_value,  hospital, rate, year) values(%s,%s,%s,%s,%s,%s,%s)'
            mycursor.execute(sql, (rank, province, city, scientific_value, hospital_name, rate, year))
    conn.commit()
    mycursor.close()
    conn.close()
...
def handler_url():
    for i in range(2009, 2019):
        url = "http://rank.cn-healthcare.com/rank/profession-scientific-best/" + str(i)
